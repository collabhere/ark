...
export const FormSectionSSH: FC<AdvancedConnectionFormProps> = (props) => {
	...
	const editSSHDetails = useCallback(
		function <T extends Ark.StoredConnection["ssh"]>(key: keyof T, value: T[keyof T]) {
				...
				<FormGroup helperText="When enabled, any configurations made in the 'Connection' section will be ignored.">
					<div className="input-field">
						<Checkbox
							...
							onChange={() => editSSHDetails("useSSH", !connectionData.ssh.useSSH)}
				...
				<FormGroup label="MongoDB Host">
					<div className="input-field">
						<InputGroup
							...
							disabled={!connectionData.ssh.useSSH}
							onChange={(e) => editSSHDetails("mongodHost", e.target.value)}
						/>
					</div>
				</FormGroup>
				<FormGroup label="MongoDB Port">
					<div className="input-field">
						<InputGroup
							...
							disabled={!connectionData.ssh.useSSH}
							onChange={(e) => editSSHDetails("mongodPort", e.target.value)}
						/>
					</div>
				</FormGroup>
			</div>
			<FormGroup label="Username">
				<div className="input-field">
					<InputGroup
						...
						disabled={!connectionData.ssh.useSSH}
						onChange={(e) => editSSHDetails("username", e.target.value)}
					/>
				</div>
			</FormGroup>
				...
				<FormGroup label="Password">
					<div className="input-field">
						<InputGroup
							...
							disabled={!connectionData.ssh.useSSH}
							onChange={(e) => editSSHDetails("password", e.target.value)}
						/>
					</div>
				</FormGroup>
			)}
			{connectionData.ssh.method === "privateKey" && (
				<FormGroup label="Private Key" helperText="Enter your private key here">
					<div className="input-field">
						<TextArea
							value={connectionData?.ssh?.privateKey}
							disabled={!connectionData.ssh.useSSH}
							onChange={(e) => editSSHDetails("privateKey", e.target.value)}
						/>
					</div>
				</FormGroup>
			)}
			{connectionData.ssh.method === "privateKey" && (
				<FormGroup label="Passphrase" helperText="Optional key passphrase">
					<div className="input-field">
						<InputGroup
							value={connectionData?.ssh?.method}
							disabled={!connectionData.ssh.useSSH}
							onChange={(e) => editSSHDetails("passphrase", e.target.value)}
						/>
					</div>
				</FormGroup>
			)}
