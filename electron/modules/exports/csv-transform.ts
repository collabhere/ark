...
const formatRow = (headers: Set<string>, cells: CsvCell[], delim = ",") =>
	[...headers].map((head) => cells.find((cell) => cell.header === head)?.value || "").join(delim) + "\n";
...
const addCells = (
	...
	for (const [key, value] of Object.entries(document)) {
		if (Array.isArray(value)) {
			value.forEach((x, idx) => addCells(x, cells, headers, [...path, String(key), String(idx)], onlyHeaders));
...
export const CSVTransform = (options: ImportOptions) => {
	...
	return new Transform({
		...
		transform(chunk: Record<string, any>, encoding: string, callback: (err?: Error, value?: { row: string }) => void) {
			try {
				const cells: CsvCell[] = [];
