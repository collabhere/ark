...
import { CreateShell, DestroyShell, ExportData, InvokeJS, ShellEvalResult, StoredShellValue } from "./types";
...
export function createShellManager(params: CreateShellParams) {
	const { store: shells, driver } = params;
	...
	const manager = {
		create: async (data: CreateShell) => {
			...
			const mongoOptions = {
				...
				replicaSet:
					connectionInfo.replicaSetDetails && connectionInfo.replicaSetDetails.set
		...
		eval: async (data: InvokeJS) => {
			...
			const evalResult = await shell.evaluator.evaluate(data.code, shell.database, {
				page: data.page,
			...
			if (
				Array.isArray(evalResult.result) &&
				evalResult.result.every((document) => document._id && isObjectId(document._id))
			) {
